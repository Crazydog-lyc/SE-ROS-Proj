...
RUN_HEADLESS="${RUN_HEADLESS:-$(profile_setting run_headless True)}"
ENABLE_SAFETY="${ENABLE_SAFETY:-$(profile_setting enable_safety False)}"
ENABLE_SEMANTIC="${ENABLE_SEMANTIC:-$(profile_setting enable_semantic False)}"
...
WORLD_FILE="${WORLD_FILE:-$(profile_setting world_file demo_pillar_room.sdf)}"
...
if [[ "${GENERATE_SCATTERED_WORLD}" == "True" || "${GENERATE_SCATTERED_WORLD}" == "true" || "${GENERATE_SCATTERED_WORLD}" == "1" ]]; then
  if [[ "${WORLD_FILE}" != "demo_pillar_room.sdf" ]]; then
    echo "WARNING: GENERATE_SCATTERED_WORLD is enabled but WORLD_FILE=${WORLD_FILE}; generated demo_pillar_room.sdf will not be used." >&2
  fi
...
/usr/bin/python3.10 "${SCRIPT_DIR}/run_batch.py" \
  ...
  --extra-launch-arg "world_file:=${WORLD_FILE}" \
