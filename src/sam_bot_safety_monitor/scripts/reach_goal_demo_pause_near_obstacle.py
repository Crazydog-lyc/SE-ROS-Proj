...
def main():
    ...
    navigator = SafetyAwareNavigator(
        node_name="reach_goal_demo_pause_near_obstacle_navigator"
    )

    print("[DEMO B] safety stop demonstration near obstacle", flush=True)
    print(
        "[DEMO B] goal is beyond the pillar; monitor should pause before collision",
        flush=True,
    )
    ...
    while rclpy.ok():
        ...
        if safety_state == "PAUSED":
            print(
                "[DEMO B] safety monitor paused the robot before collision",
                flush=True,
            )
        ...
        if feedback and feedback_counter % 5 == 0:
            ...
            print(
                f"[DEMO B] approaching the pillar-side goal, ETA {eta:.0f}s",
                flush=True,
            )
    ...
    if demo_pause_completed:
        print(
            "[DEMO B] safety-stop demo completed with obstacle pause handling",
            flush=True,
        )
        raise SystemExit(0)
